...
<body>
	...
	<div id="QueryArea">
		<form
			action="${pageContext.request.contextPath}/FoodTypeServlet?method=search"
			method="get">
			<input type="hidden" name="method" value="search"> <input
				type="text" name="foodTyepName" title="请输入菜系名称"> <input
			...
			<tbody id="TableData">
				...
				<c:choose>
					<c:when test="${not empty requestScope.foodList }">
						<c:forEach var="foodType" items="${ requestScope.foodList}" varStatus="vs">
							<tr>
								<td>${vs.count}</td>
								<td>${foodType.typeName }</td>
								<td><a href="${pageContext.request.contextPath}/FoodTypeServlet?id=${foodType.id}&method=viewUpdate" class="FunctionButton">更新</a>
									<a href="${pageContext.request.contextPath}/FoodTypeServlet?id=${foodType.id}&method=delete" class="FunctionButton">删除</a></td>
							</tr>
						</c:forEach>
					</c:when>
					<c:otherwise>
						<tr>
							<td colspan="3">没有此数据</td>
						</tr>
					</c:otherwise>
				</c:choose>
				<!-- 
					<tr>
						<td>1</td>
						<td>粤菜</td>
						<td><a href="updateCuisine.html" class="FunctionButton">更新</a>
