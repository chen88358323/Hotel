	...
	<div id="MainArea">
		<table class="MainArea_Content" cellspacing="0" cellpadding="0">
			...
			<tbody id="TableData">
				...
				<tr class="TableDetail1">
					...
					<td><a
						href="/wirelessplatform/board.html?method=update&id=1&isBook=0"
			 ...
			 -->
				<c:choose>
					<c:when test="${not empty requestScope.dinnerTableList }">
						<c:forEach var="dinnerTable"
							items="${requestScope.dinnerTableList}" varStatus="vs">
							<tr class="TableDetail1">
								<td align="center">${vs.count}</td>
								<td align="center">${dinnerTable.tableName}</td>
								<td align="center"><c:if
										test="${dinnerTable.tableStatus==0 }">
										空闲
									</c:if> <c:if test="${dinnerTable.tableStatus==1 }">
										预定
									</c:if></td>
		...
		<div id="TableTail" align="center">
			<div class="FunctionButton">
				<a
					href="${pageContext.request.contextPath}/sys/table/dinnertable_save.jsp">添加</a>
			</div>
		</div>
	</div>
</body>
